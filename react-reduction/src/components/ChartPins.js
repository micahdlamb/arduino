...
const ChartPins = ({ title, datasets, options, history, freq, ...restProps }) => {
  ...
  useEffect(() => {
    ...
    let id = setInterval(async () => {
      await Promise.all(datasets.map(async curve => {
        let value = await curve.chip.readPin(curve.pin)
...
ChartPins.defaultProps = {
  history: 10,
  freq: 1
};

export default React.memo(ChartPins);
